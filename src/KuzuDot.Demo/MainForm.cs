...
namespace KuzuDot.Demo
{
    internal class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            try
            {
                _connection.NonQuery("CREATE NODE TABLE Person(id STRING, name STRING, age INT64, PRIMARY KEY(id))").Dispose();
                _connection.NonQuery("CREATE (:Person {id:'1', name:'Alice', age:30})").Dispose();
                _connection.NonQuery("CREATE (:Person {id:'2', name:'Bob', age:36})").Dispose();
        ...
        private void ExecuteQuery()
        {
            ...
            try
            {
                ...
                while (result.HasNext())
                {
                    ...
                    {
                        if (c > 0) sb.Append(" | ");
                        using var val = row.GetValue(c); // concrete subtype of KuzuValue
            ...
            // Unexpected exceptions bubble to WinForms handler (avoid swallowing)
        }
