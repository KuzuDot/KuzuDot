namespace KuzuDot.Tests.ResourceManagement
{
    ...
    public sealed class ResourceManagementTests : IDisposable
    {
        ...
        public void ExplicitDisposal_InCorrectOrder_ShouldNotThrow()
        {
            ...
            using (var statement = connection.Prepare("CREATE (:TestNode {id: $id});"))
            {
            ...
            TestContext?.WriteLine("Resources created and disposed deterministically via using scopes");
        }
        ...
        public void ExplicitDisposal_InWrongOrder_ShouldStillWork()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(result.IsSuccess);
                TestContext?.WriteLine("All resources created successfully");
            }
            finally
            {
                // Dispose in wrong order (database first) - should still work due to proper implementation
                TestContext?.WriteLine("Disposing resources in wrong order (testing robustness)...");

                database?.Dispose();
                TestContext?.WriteLine("Database disposed first");

                connection?.Dispose();
                TestContext?.WriteLine("Connection disposed after database");

                result?.Dispose();
                TestContext?.WriteLine("QueryResult disposed last");
            }

            TestContext?.WriteLine("Test completed - implementation handles wrong disposal order gracefully");
        }
        ...
        public void DoubleDispose_ShouldNotThrow()
        {
            ...
            // First disposal
            TestContext?.WriteLine("First disposal...");
            result.Dispose();
            statement.Dispose();
            connection.Dispose();
            database.Dispose();
            TestContext?.WriteLine("First disposal completed");

            // Second disposal should not throw
            TestContext?.WriteLine("Second disposal (should be safe)...");
            result.Dispose();
            statement.Dispose();
            connection.Dispose();
            database.Dispose();
            TestContext?.WriteLine("Second disposal completed without exceptions");
        }
        ...
        public void ResourcesAfterDispose_ShouldThrowObjectDisposedException()
        {
            ...
            TestContext?.WriteLine("Testing database usage after disposal...");
            Assert.ThrowsExactly<ObjectDisposedException>(() => database.Connect());
            TestContext?.WriteLine("Database correctly throws ObjectDisposedException after disposal");
            ...
            TestContext?.WriteLine("Testing connection usage after disposal...");
            Assert.ThrowsExactly<ObjectDisposedException>(() => connection.Query("SELECT 1;"));
            TestContext?.WriteLine("Connection correctly throws ObjectDisposedException after disposal");
        }
        ...
        public void ExceptionDuringResourceUsage_ShouldNotPreventProperCleanup()
        {
            ...
            using (var database = Database.FromMemory())
            {
                ...
                try
                {
                    ...
                    using var _ = connection.Query("INVALID SQL THAT WILL THROW EXCEPTION");
                ...
                catch (KuzuException ex)
                {
                    ...
                    TestContext?.WriteLine($"Expected KuzuException caught: {ex.Message}");
                }
            }
            Assert.IsTrue(exceptionThrown, "Exception should have been thrown and caught");
            TestContext?.WriteLine("Resources cleaned up via using after exception");
        }
        ...
        public void MultipleConnections_FromSameDatabase_ShouldManageIndependently()
        {
            ...
            try
            {
                ...
                using (var result2 = connection2.Query("CREATE (:MultiConnTest {id: 2});"))
                {
                    ...
                    TestContext?.WriteLine("Both connections work independently");
                }

                // Dispose one connection
                connection1.Dispose();
                connection1 = null;
                TestContext?.WriteLine("First connection disposed");

                // Second connection should still work
                using (var result = connection2.Query("MATCH (n:MultiConnTest) RETURN count(n);"))
                {
                    Assert.IsTrue(result.IsSuccess);
                    TestContext?.WriteLine("Second connection still works after first was disposed");
                }
            }
            finally
            {
                connection2?.Dispose();
                TestContext?.WriteLine("All connections cleaned up");
            }
        }
        ...
        public void LargeResultSet_WithProperDisposal_ShouldNotLeakMemory()
        {
            ...
            using (var queryResult = connection.Query("MATCH (n:LargeTest) RETURN n.id, n.data ORDER BY n.id;"))
            {
                ...
                Assert.AreEqual(2UL, queryResult.ColumnCount); // Should have 2 columns: id and data
                Assert.IsTrue(queryResult.HasNext()); // Should have at least one result

                Console.WriteLine($"? Retrieved query result with {queryResult.ColumnCount} columns and confirmed data exists");
            }
