...
namespace KuzuDot.Examples
{
    ...
    /// </summary>
    internal static class ComprehensiveExample
    {
        public static void Run()
        {
            ...
            using var database = ExampleLog.SafeCreate("Create In-Memory Database", () => Database.FromMemory());
        ...
        private static void CreateTestSchema(Connection connection)
        {
            ...
            Ensure(result.IsSuccess, () => $"Failed to create schema: {result.ErrorMessage}");
            ...
            int age, float height, double weight, bool isActive, DateTime birthDate, DateTime createdAt)
        {
            ...
            Ensure(result.IsSuccess, () => $"Insert failed for '{name}': {result.ErrorMessage}");
        ...
        private static void RetrieveAndCalculate(Connection connection)
        {
            ...
            Ensure(result.IsSuccess, () => $"Query failed: {result.ErrorMessage}");
            ...
            while (result.HasNext())
            {
                ...
                var id = ((KuzuInt64)idValue).Value;
                var name = ((KuzuString)nameValue).Value;
                var age = ((KuzuInt32)ageValue).Value;
                var heightFt = ((KuzuFloat)heightValue).Value;
                var weightLbs = ((KuzuDouble)weightValue).Value;
                var active = ((KuzuBool)isActiveValue).Value;
                var birthDateStr = ((KuzuDate)birthDateValue).ToString();
                var createdMicros = ((KuzuTimestamp)createdAtValue).UnixMicros;
            ...
            if (count > 0)
            {
                // TODO: use query to gather aggregates directly
        ...
        {
            if (!condition) throw new InvalidOperationException(errorFactory());
        }
    }
}